...
if (RUN_PROCESSING_AND_SUBSET) {
  ...
  plotting_data <- rbind(
    ...
    # normalized_data_set_I_do_not_need
...
if (RELATIVE_SECRETION) {
  ...
  ELISA_PLOT <- ggplot(data = plotting_stats_relative, aes(x = Relative_Intensity_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR, pattern = CONDITION, group = rev(CONDITION))) +
    ...
    theme(axis.text.x       = element_text(size = SIZE, vjust = 0.6),
          ...
          legend.key.size   = unit(9, "mm")) ; ELISA_PLOT
...
if (REAL_SECRETION) {
  ...
  # # ## add zoom to the plot if you want to focus on a specific range (a)
  # # zoom_max <- 0.2 ; ELISA_PLOT_REAL + facet_zoom(xlim = c(0, zoom_max), zoom.data = ifelse(a <= zoom_max, NA, FALSE))
  # 
  # # ## or plot the data however you like
  # plotting_means$CL_NAME_ON_PLOT <- reorder(plotting_means$CL_NAME_ON_PLOT, -plotting_means$ORDER_NO)
  # plotting_stats_real$CL_NAME_ON_PLOT <- reorder(plotting_stats_real$CL_NAME_ON_PLOT, -plotting_stats_real$ORDER_NO)
  # 
  # ELISA_PLOT <- ggplot(data = plotting_stats_real, aes(x = IL2_concentration_Dilution_Factor_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR, pattern = CONDITION, group = rev(CONDITION))) +
...
if (FOLD_CHANGE) {
  ...
  fold_change_data <- plotting_data %>%
    ...
    ###########
    ###########
    # special case because the controls were generated in separate experiments and on separate days by separate people and different control cell batches
    # usually you can group by Date, but here that' insufficient because both experiments were done on the same day
    # just hash out the following line or adjust it to your needs
    mutate(EXPERIMENT = ifelse(Plate %in% c(1, 2), "exp1", "exp2")) %>%
    ###########
    ###########
  
    group_by(CL_NUMBER, Date, EXPERIMENT) %>%
  ...
  FC_ELISA_PLOT <- ggplot(data = fold_change_data, aes(x = fold_change_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR)) +
    ...
    annotate("text", x = 2.1, y = max(as.numeric(as.factor(fold_change_data$CL_NAME_ON_PLOT))) + 0.5, 
             label = "2X", vjust = -0.5, hjust = 0, size = TEXT-1) +
    ...
    # Adjust y-axis limits to make space for annotation
    coord_cartesian(clip = "off", ylim = c(NA, max(as.numeric(as.factor(fold_change_data$CL_NAME_ON_PLOT))) + 0.8)) ; FC_ELISA_PLOT
