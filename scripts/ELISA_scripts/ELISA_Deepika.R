...
if (RUN_SETTINGS_AND_PREP) {
  ...
  Input_Directory <- ifelse(dir.exists(file.path("/Volumes/TAYLOR-LAB/Finn_v2/Other_Project/DEEPIKA/", figure)),
                            file.path("/Volumes/TAYLOR-LAB/Finn_v2/Other_Project/DEEPIKA/", figure), 
                            file.path(figure))
...
if (RUN_PROCESSING_AND_SUBSET) {
  ...
  ELISA_DATA            <- plate_data
  ELISA_DATA_normalized <- process_ELISA_data(DF = ELISA_DATA, NEGATIVE_CTRL = "cl204", POSITIVE_CTRL = "cl069")
  
  # check completeness
  table(ELISA_DATA_normalized$CELL_LINE, ELISA_DATA_normalized$STIM_DAY)
  
  plotting_data        <- ELISA_DATA_normalized
...
if (RELATIVE_SECRETION) {
  ...
  if (nrow(stat_significance_dt) > 0) {
    ...
    ELISA <- ggplot(data = plotting_stats, aes(x = Relative_Intensity_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR, pattern = CONDITION, group = rev(CONDITION))) +
    ...
    ELISA <- ggplot(data = plotting_means, aes(x = Relative_Intensity_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR, pattern = CONDITION, group = rev(CONDITION))) +
      geom_col(aes(col = PLOTTING_COLOR), position = position_dodge(width = 0.7), width = 0.68, alpha = 0.5) +
      geom_point(data = plotting_means, aes(x = Relative_Intensity_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR), 
                 col = "black", shape = 21, size = POINTS, position = position_jitterdodge(dodge.width = 0.5, jitter.width = 0.4, seed = 750), show.legend = FALSE) +
      scale_fill_manual(name  = "CL_NAME_ON_PLOT", values = plotting_means$PLOTTING_COLOR, breaks = plotting_means$PLOTTING_COLOR, labels = ifelse(plotting_means$CONDITION == "UNSTIM", paste0("- ", plotting_means$STIMULANT), paste0("+ ", plotting_means$STIMULANT))) +
...
if (REAL_SECRETION) {
  ...
  if (nrow(stat_significance_dt) > 0) {
    ...
    # plotting relative values
    ELISA_real <- ggplot(data = plotting_stats, aes(x = IL2_concentration_Dilution_Factor_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR, pattern = CONDITION, group = rev(CONDITION))) +
      geom_col(aes(col = PLOTTING_COLOR), position = position_dodge(width = 0.7), width = 0.68, alpha = 0.5) +
      geom_errorbar(aes(y = CL_NAME_ON_PLOT,
    ...
    ELISA_real <- ggplot(data = plotting_means, aes(x = IL2_concentration_Dilution_Factor_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR, pattern = CONDITION, group = rev(CONDITION))) +
      geom_col(aes(col = PLOTTING_COLOR), position = position_dodge(width = 0.7), width = 0.68, alpha = 0.5) +
      geom_point(data = plotting_means, aes(x = IL2_concentration_Dilution_Factor_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR), 
                 col = "black", shape = 21, size = POINTS, position = position_jitterdodge(dodge.width = 0.5, jitter.width = 0.4, seed = 750), show.legend = FALSE) +
      scale_fill_manual(name  = "CL_NAME_ON_PLOT", values = plotting_means$PLOTTING_COLOR, breaks = plotting_means$PLOTTING_COLOR, labels = ifelse(plotting_means$CONDITION == "UNSTIM", paste0("- ", plotting_means$STIMULANT), paste0("+ ", plotting_means$STIMULANT))) +
...
if (RELATIVE_SECRETION_ZOOM) {
  ...
  if (nrow(stat_significance_dt) > 0) {
    ...
    # plotting relative values
    ELISA_zoom <- ggplot(data = plotting_stats, aes(x = Relative_Intensity_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR, pattern = CONDITION, group = rev(CONDITION))) +
      geom_col(aes(col = PLOTTING_COLOR), position = position_dodge(width = 0.7), width = 0.68, alpha = 0.5) +
      geom_errorbar(aes(y = CL_NAME_ON_PLOT,
                        xmin = Relative_Intensity_mean - Relative_Intensity_sem,
                        xmax = Relative_Intensity_mean + Relative_Intensity_sem),
    ...
    # plotting relative values
    ELISA_zoom <- ggplot(data = plotting_means, aes(x = Relative_Intensity_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR, pattern = CONDITION, group = rev(CONDITION))) +
      geom_col(aes(col = PLOTTING_COLOR), position = position_dodge(width = 0.7), width = 0.68, alpha = 0.5) +
      geom_point(data = plotting_means, aes(x = Relative_Intensity_mean, y = CL_NAME_ON_PLOT, fill = PLOTTING_COLOR), col = "black",
                 shape = 21, size = POINTS, position = position_jitterdodge(dodge.width = 0.5, jitter.width = 0.4, seed = 750), show.legend = FALSE) +
      scale_y_discrete(expand = c(0, 0)) +
...
if (SAVE) {
  ...
  if (RELATIVE_SECRETION)      {ggsave(file.path(Output_Directory, "ELISA_rltv.svg"), plot = ELISA,      device = "svg", width = 12, height = 7)}
  if (RELATIVE_SECRETION_ZOOM) {ggsave(file.path(Output_Directory, "ELISA_zoom.svg"), plot = ELISA_zoom, device = "svg", width = 14, height = 21)}
  if (REAL_SECRETION)          {ggsave(file.path(Output_Directory, "ELISA_real.svg"), plot = ELISA_real, device = "svg", width = 12, height = 7)}
