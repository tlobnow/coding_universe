...
# parameters_path = "/Volumes/TAYLOR-LAB/Finn/new_pipeline/pending_processing/20230616_batch/Input/parameter_tables/"
# parameters_path = "/Volumes/TAYLOR-LAB/Finn/new_pipeline/pending_processing/20230619_batch/Input/parameter_tables/"
# parameters_path = "/Volumes/TAYLOR-LAB/Finn/new_pipeline/pending_processing/20230623_batch/Input/parameter_tables/"
# parameters_path = "~/Desktop/20230619_batch/Input/parameter_tables/"
# parameters_path = "~/Documents/nd2/LCI/20231201_batch/Input/parameter_tables/"
# parameters_path = "~/Documents/PRESENTATIONS/20240307_batch/Input/parameter_tables/"
...
# directories_list$path <- gsub("/cobra/u/flobnow/new_pipeline/pending_processing/", "/Users/u_lobnow/Documents/PRESENTATIONS/", x = directories_list$path)
...
# summary_path = "/Volumes/TAYLOR-LAB/Finn/new_pipeline/pending_processing/batch_2_20230602/Processing/06_Colocalization/BDLD_10H-MyD88-TIR-TRAF6-BD-GFP TRAF6/20230602 6nM_cl320-BDLD10H_TRAF6_MyD88_001/summmary.csv"
# summary_path = "/Users/u_lobnow/Documents/PRESENTATIONS/20240307_batch/Processing/06_Colocalization/cl069_MyD88 TRAF6/20240307 3nM_cl069_MyD88_TRAF6 001/summmary.csv"
# summary_path = "/Users/u_lobnow/Documents/PRESENTATIONS/20240307_batch/Processing/06_Colocalization/cl236_MyD88 TRAF6/20240307 3nM cl236_MyD88_TRAF6_DHF91 001/summmary.csv"
# summary_path = "/Users/u_lobnow/Documents/PRESENTATIONS/20240307_batch/Processing/06_Colocalization/cl236_MyD88 TRAF6/20240307 3nM cl236_MyD88_TRAF6_DHF91 002/summmary.csv"
# summary_path = "/Users/u_lobnow/Documents/PRESENTATIONS/20240307_batch/Processing/06_Colocalization/cl425_MyD88 TRAF6/20240307 6nM_cl425_MyD88_TRAF6_12xTA 001/summmary.csv"
# summary_path = "/Users/u_lobnow/Documents/PRESENTATIONS/20240307_batch/Processing/06_Colocalization/cl426_MyD88 TRAF6/20240307 6nM_cl426_MyD88_TRAF6_15xTA 001/summmary.csv"
# summary_path = "/Users/u_lobnow/Documents/PRESENTATIONS/20240307_batch/Processing/06_Colocalization/cl255_MyD88 TRAF6/20240307 3nM_cl255_MyD88_TRAF6_3x 001/summmary.csv"
# summary_path = "/Users/u_lobnow/Documents/PRESENTATIONS/20240307_batch/Processing/06_Colocalization/cl255_MyD88 TRAF6/20240307 3nM_cl255_MyD88_TRAF6_3x 002/summmary.csv"
# summary_path = "/Users/u_lobnow/Documents/PRESENTATIONS/20240307_batch/Processing/06_Colocalization/cl255_MyD88 TRAF6/20240307 3nM_cl255_MyD88_TRAF6_3x 003/summmary.csv"
...
ConvertUnits <- function(x) {
  # Use ifelse to handle the conditions in a vectorized way
  y <- ifelse(x == "ms", 10^-3, 1)
  return(y)
}
...
# Prep up
file_list <- file_list %>% 
  mutate(image = dirname(dirname(protein_relative_path)),
  ...
  select(-c(exposure_units, exposure_measurement)) %>% 
  as.data.table()

# Extract the directory path from the summary_path
# directory_path <- dirname(summary_path)

# Specify the file pattern for calibration files
# cal_list_paths   <- list.files(directory_path, pattern = "_calibration.csv.gz", full.names = TRUE)
# cal_list         <- lapply(cal_list_paths, fread)
# calibration_list <- rbindlist(cal_list)
...
QuantifyCalibration <- function(FileX){
  ...
  Table <- Table %>% 
    summarize(SD = mad(TOTAL_INTENSITY),
...
# Path <- dirname(image_list$protein_relative_path)
# Path <- file.path(colocalization_path, Path)
# Path <- file.exists(Path)
# image_list <- image_list[Path]
...
GetCalibrationImages <- function(ImageX){
  tryCatch({
    ...
    temp_calibration_list <- calibration_list %>% 
      # Get the correct channel
      filter(channel == temp_image_list$channel) %>% 
      as.data.table()
    
    if(NROW(temp_calibration_list)>0){
      
      temp_calibration_list <- temp_calibration_list %>% 
        # Get the nearest power
        mutate(power_test =  abs(power - temp_image_list$power)) %>% 
        filter(power_test == min(power_test)) %>% 
        # Get the nearest exposure
        mutate(exposure_test =  abs(exposure - temp_image_list$exposure)) %>% 
        filter(exposure_test == min(exposure_test)) %>% 
        # Get the nearest direction
        mutate(direction_test =  abs(direction - temp_image_list$direction)) %>% 
        filter(direction_test == min(direction_test)) %>% 
        # Get the nearest angle
        mutate(angle_test = abs(angle - temp_image_list$angle)) %>% 
        filter(direction_test == min(direction_test)) %>% 
        # Get the nearest date
        mutate(
          # date_test =  abs(date - temp_image_list$date)
          date_test = as.numeric(difftime(date, temp_image_list$date, units = "secs"))
          ) %>% 
        filter(date_test == min(date_test)) %>% 
        # Lowest SEM
        filter(SEM == min(SEM)) %>% 
        as.data.table()
...
PairedList <-  lapply(1:NROW(image_list), GetCalibrationImages) 
#PairedList <-  lapply(1:NROW(image_list), GetCalibrationImages, mc.cores = detectCores(logical = T))
...
CombineCellTables <- function(CellX){
  tryCatch({
    ...
    # Get tables
    CellPairedCalibrations <- PairedList %>%
      filter(CELL_PATH == Cells[CellX]) %>% 
      mutate(PROTEIN = PROTEIN_NAME) %>% 
      select(PROTEIN, CALIBRATION_IMAGE, CALIBRATION_TOTAL_INTENSITY, CALIBRATION_STANDARD_DEVIATION) %>% 
      as.data.table()
    ...
    if(file.exists(ColocalizationTablePath)){
      ...
      SelectIntensityTables <- IntensityTables %>% 
        mutate(COMPLEMENTARY_UNIVERSAL_SPOT_ID    = UNIVERSAL_SPOT_ID,
               COMPLEMENTARY_NORMALIZED_INTENSITY = NORMALIZED_INTENSITY) %>% 
        select(COMPLEMENTARY_UNIVERSAL_SPOT_ID, COMPLEMENTARY_NORMALIZED_INTENSITY) %>% 
        as.data.table()
      ## Add to table
      ColocalizationTable <- merge(ColocalizationTable, SelectIntensityTables, by = "COMPLEMENTARY_UNIVERSAL_SPOT_ID")
      
      ColocalizationTable <- ColocalizationTable %>% 
        pivot_wider(names_from  = PROTEIN_ID,
                    id_cols     = UNIVERSAL_SPOT_ID,
    ...
    NearestNeighborTable <- IntensityTables %>% 
      filter(!is.na(POSITION_X)) %>% 
      group_by(PROTEIN, FRAME) %>% 
      mutate(N = n()) %>% 
      filter(N > 1) %>% 
      select(PROTEIN, FRAME, PUNCTA_DIAMETER, UNIVERSAL_SPOT_ID, POSITION_X, POSITION_Y) %>% 
      ungroup() %>% 
      as_tibble() %>% 
      group_split(PROTEIN, FRAME)
    ...
    if(file.exists(TimeTable)){
      ...
      IntensityTables <- IntensityTables %>% 
    ...
    IntensityTables <- IntensityTables %>% 
      ...
      # Rounds to nearest half log
      # Classifies ligands based on log using base 3.162 (10^.5)
      mutate(LIGAND_DENSITY_CAT = round(log(LIGAND_DENSITY, base = (10^.5)), digits = 0) * 0.5) %>% 
      # Convert back to linear
      mutate(LIGAND_DENSITY_CAT = signif(10^LIGAND_DENSITY_CAT, 2),
             # Get frame number
      ...
      mutate(START_TO_MAX_INTENSITY = MAX_NORMALIZED_INTENSITY - STARTING_NORMALIZED_INTENSITY) %>% 
      as.data.table()
    
    CellData <- IntensityTables %>% 
      select(one_of("RELATIVE_PATH",
                    "LIGAND",
                    "LIGAND_DENSITY",
                    "CHANNEL",
                    "POWER",
    ...
    Parameterless <- IntensityTables %>% 
      select(!one_of("LIGAND",
                    "LIGAND_DENSITY",
                    "CHANNEL",
                    "POWER",
                    "EXCITATION",
    ...
    Essential <- IntensityTables %>% 
      select(one_of(c("RELATIVE_PATH",
                      "PROTEIN",
                      "COHORT",
                      "LIGAND_DENSITY_CAT",
                      "IMAGE",
                      ...
                      "SPOTS_WITHIN_RADIUS",
                      ColocalizationNames))) %>% 
      mutate(ANALYSIS_TIME_STAMP = ANALYSIS_TIME_STAMP) %>% 
      as.data.table()
...
#   print(paste("Working on", OutputX))           # Status output
#   Paths <- file.path(ProcessedImages, OutputX)  # Get table paths
#   Tables <- lapply(Paths, fread)                # Read tables
#   Tables <- rbindlist(Tables, fill = TRUE, use.names = TRUE) # Merge tables
#   save_name <- file.path(output_path, OutputX)  # Save tables
